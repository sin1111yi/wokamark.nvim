...
-- :WokaMarkManage — manage picker with shortcuts d/r/a/i
function M.manage_picker()
  ...
  snacks.pick({
    ...
    preview = picker_preview,
    actions = {
      -- d: delete the selected workspace, refresh the list
      wokamark_delete = function(picker)
        local item = picker:current()
        if not item then return end
        M._delete(item)
        picker.opts.items = decorate_items(read_index())
        picker:refresh()
      end,
      -- r: rename the selected workspace (prompt for the new name)
      wokamark_rename = function(picker)
        local item = picker:current()
        if not item then return end
        local new_name = vim.fn.input('Wokamark rename: ', item.name or '')
        if new_name == '' then
          vim.notify('Wokamark: rename aborted (empty name)', vim.log.levels.INFO)
          return
        end
        M._rename(item, new_name)
        picker.opts.items = decorate_items(read_index())
        picker:refresh()
      end,
      -- a: mark the current path; close the picker first so the saved
      -- session never contains the picker float, then reopen fresh
      wokamark_add = function(picker)
        picker:close()
        M.mark({})
      ...
      -- i: full details of the selected workspace in the preview pane
      wokamark_info = function(picker)
        local item = picker:current()
        if not item then return end
        picker.preview:set_title('Details: ' .. display_name(item))
        picker.preview:set_lines({
          'name:      ' .. (item.name or '-'),
          'cwd:       ' .. (item.cwd or '-'),
          'repo:      ' .. (item.repo or '-'),
          'branch:    ' .. (item.branch or '-'),
          'commit:    ' .. (item.hash or '-'),
          'path hash: ' .. (item.path_hash or '-'),
          'last used: ' .. (item.last_used or '-'),
